...
def _write_short_cover_letter(role: RoleType, content: TailoredContent, output_dir: Path) -> Path | None:
    """
    Write the plain-text short cover letter (≤1500 chars) for pasting into
    character-limited application portals (e.g. France Travail). Always
    generated alongside the PDF letter — never LaTeX, no compilation needed.
    """
    if not content.cl_short:
        logger.warning("No cl_short generated by the LLM — skipping short cover-letter .txt")
        return None

    role_label = _canonical_role_label(str(role))
    lang_tag = (content.language or "en").lower()
    short_label = "LettreMotivation_Courte" if lang_tag == "fr" else "CoverLetter_Short"
    short_txt_name = f"{CV_OWNER_SLUG}-{short_label}_{role_label}_{lang_tag}.txt"
    short_txt_path = output_dir / short_txt_name
    short_txt_path.write_text(content.cl_short, encoding="utf-8")

    char_count = len(content.cl_short)
    if char_count > 1500:
        logger.warning(f"Short cover letter is {char_count} chars — over the 1500-char portal limit")
    logger.info(f"Short cover letter written ({char_count} chars): {short_txt_path}")
    return short_txt_path
